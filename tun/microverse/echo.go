...
func NewEchoServer(a Addr, count bool) *EchoServer {
	if a.Port == 0 {
		a.Port = GetAvailPort()
	}
	if a.Ip == "" {
		...
		for {
			...
			go func(netconn net.Conn) {
				...
				for {
					...
					err = netconn.SetReadDeadline(time.Now().Add(2000 * time.Millisecond))
					panicOn(err)
					...
					if n > 0 {
						...
						err = netconn.SetWriteDeadline(time.Now().Add(2000 * time.Millisecond))
						panicOn(err)

						netconn.Write(buf[:n])
						if r.count {
							netconn.Write([]byte(fmt.Sprintf("%d", echoCount)))
						}
						echoCount++
						po("echoCount = %d", echoCount)
					}
