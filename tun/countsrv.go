...
func (r *CountingTestClient) ShowTmHistory() {
	...
	for i := 0; i < min; i++ {
		fmt.Printf("CountingTestClient history: elap: '%s'    Send '%v'   Recv '%v'  \n", r.tmLastRecv[i].Sub(r.tmLastSend[i]), r.tmLastSend[i], r.tmLastRecv[i])

	}
	for i := 0; i < min-1; i++ {
		fmt.Printf("CountingTestClient history: send-to-send elap: '%s'\n", r.tmLastSend[i+1].Sub(r.tmLastSend[i]))
	}
	for i := 0; i < min-1; i++ {
		fmt.Printf("CountingTestClient history: recv-to-recv elap: '%s'\n", r.tmLastRecv[i+1].Sub(r.tmLastRecv[i]))
	}
...
func (r *CountingTestServer) ShowTmHistory() {
	...
	for i := 0; i < min; i++ {
		fmt.Printf("CountingTestServer history: elap: '%s'    Recv '%v'   Send '%v'  \n", r.tmLastSend[i].Sub(r.tmLastRecv[i]), r.tmLastRecv[i], r.tmLastSend[i])
	}

	for i := 0; i < min-1; i++ {
		fmt.Printf("CountingTestServer history: send-to-send elap: '%s'\n", r.tmLastSend[i+1].Sub(r.tmLastSend[i]))
	}
	for i := 0; i < min-1; i++ {
		fmt.Printf("CountingTestServer history: recv-to-recv elap: '%s'\n", r.tmLastRecv[i+1].Sub(r.tmLastRecv[i]))
	}
